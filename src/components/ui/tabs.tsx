...
>(({ className, rightElementSlot, children, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      "flex w-full grow items-center justify-start gap-x-4 border-b border-border",
      className,
    )}
    {...props}
  >
    {children}
    {rightElementSlot && (
      <div className="flex w-full grow justify-end">{rightElementSlot}</div>
    )}
  </TabsPrimitive.List>
));
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Trigger
    ...
    className={cn(
      ...
      "data-[state=active]:border-blue-500 data-[state=active]:font-semibold data-[state=active]:text-foreground",
      className,
    )}
    {...props}
  />
